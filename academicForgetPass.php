...
if (isset($_GET["email2"])) {
  ...
  $resultSet = Database::search("SELECT * FROM `academic_officer` WHERE `email` = '" . $email . "';");
  $num = $resultSet->num_rows;

  if ($num == 1) {
    $code = uniqid();

    Database::insUpdelete("UPDATE `academic_officer` SET `verification_code` = '" . $code . "' WHERE `email` = '" . $email . "';");
    ...
    $mail->isHTML(true);
    $mail->Subject = 'e- Sewana Online Portal forgot Password Academic_officer verification_code';
