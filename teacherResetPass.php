...
if (empty($email)) {
  ...
  $resultSet = Database::search("SELECT * FROM `teacher` WHERE `email` = '" . $email . "' AND `verification_code` = '" . $vc . "';");
  $num = $resultSet->num_rows;

  if ($num == 1) {
    Database::insUpdelete("UPDATE `teacher` SET `password` = '" . $npw . "' WHERE `email` = '" . $email . "';");
    echo ("success");
  } else {
