...
if (empty($email)) {
  ...
  $resultSet = Database::search("SELECT * FROM `academic_officer` WHERE `email` = '" . $email . "' AND `verification_code` = '" . $vc . "';");
  $num = $resultSet->num_rows;

  if ($num == 1) {
    Database::insUpdelete("UPDATE `academic_officer` SET `password` = '" . $npw . "' WHERE `email` = '" . $email . "';");
    echo ("success");
  } else {
